...
describe 'splunk' do
  on_supported_os.reject { |_, f| f[:os]['family'] == 'windows' }.each do |os, os_facts|
    context "on #{os}" do
      let(:facts) { os_facts }
      ...
      context 'with saml auth' do
        ...
        it {
          is_expected.to contain_file('/opt/splunk/etc/apps/puppet_common_auth_saml_base/local/authentication.conf')
            .with_content(/idpSLOUrl = https:\/\/sso.internal.corp.example\/adfs\/ls\?wa=wsignout1.0/)
        }
      ...
      context 'with license server and pool suggestion' do
        ...
        it {
          is_expected.to contain_file('/opt/splunk/etc/apps/puppet_common_license_client_base/local/server.conf')
            .with_content(/master_uri = https:\/\/lm.internal.corp.example:8089\npool_suggestion = prodpool/)
        }
        # rubocop:enable Style/RegexpLiteral
      end
      ...
      context 'with search head clustering' do
        ...
        it { is_expected.to contain_package('splunk') }
        it {
          is_expected.to contain_file('/opt/splunk/etc/apps/puppet_search_shcluster_base/default/server.conf')
            .with_content(%r{conf_deploy_fetch_url = https://splunk-shd.internal.corp.example:8089})
        }
      ...
      context 'with ds_intermediate set' do
        ...
        it { is_expected.to contain_package('splunk') }
        it {
          is_expected.to contain_file('/opt/splunk/etc/apps/puppet_common_deploymentclient_base/local/deploymentclient.conf')
            .with_content(%r{repositoryLocation = /opt/splunk/etc/deployment-apps})
        }
      end
